...
export default function PublicarCasa() {
  ...
  const [fotos, setFotos] = useState([]); // lista de { file, preview }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      for (const foto of fotos) {
        const nomeLimpo = foto.file.name.replace(/[^a-zA-Z0-9._-]/g, "_");
        ...
        const {
          data: { publicUrl },
        } = supabase.storage.from("casas-fotos").getPublicUrl(nomeArquivo);

        imagens_urls.push(publicUrl);
      }
